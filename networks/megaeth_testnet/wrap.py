# SuperchainBot\networks\megaeth_testnet\wrap.py
...
async def wrap_eth(wallet_private_key, address, chain_name="megaeth_testnet", simulate=False, proxy=None):
    try:
        ...
        receipt = await sign_and_send_transaction(w3, tx, wallet_private_key, address, "Wrap ETH", simulate)
        if receipt and receipt["status"] == 1:
            return {"status": "success", "message": f"Wrapped {w3.from_wei(amount_eth, 'ether'):.6f} ETH for wallet {address[:10]}..."}
        else:
            return {"status": "error", "message": f"Failed to wrap {w3.from_wei(amount_eth, 'ether'):.6f} ETH for wallet {address[:10]}..."}
    except Exception as e:
        logger.error(f"Wrap failed for wallet {address[:10]}...: {str(e)}")
